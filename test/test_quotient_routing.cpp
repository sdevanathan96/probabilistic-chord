...
void test_quotient_routing_basic() {
    std::cout << "[1] Quotient filter routing basic..." << std::endl;

    const int N = 8;
    std::vector<NodeInfo> infos;
    std::vector<SimTransport*> transports;
    std::vector<ChordNode*> nodes;
    std::vector<QuotientRoutingFilter*> filters;
    ...
    for (int i = 0; i < N; ++i) {
        ...
        QuotientRoutingFilter* filter = new QuotientRoutingFilter(8, 24);
    ...
    int success = 0;
    for (int i = 0; i < 20; ++i) {
        uint64_t key = fnv1a_hash("quotient_routing_test_" + std::to_string(i));
...
void test_quotient_routing_with_metrics() {
    std::cout << "[2] Quotient filter routing with metrics..." << std::endl;
    ...
    for (int i = 0; i < 50; ++i) {
        uint64_t key = fnv1a_hash("metrics_quotient_" + std::to_string(i));
        nodes[0]->lookup(key);
    }
...
void test_quotient_node_leave() {
    std::cout << "[3] Quotient filter routing after node leave..." << std::endl;
    ...
    int success = 0;
    for (int i = 0; i < 10; ++i) {
        uint64_t key = fnv1a_hash("leave_quotient_" + std::to_string(i));
        NodeInfo owner = nodes[0]->lookup(key);
...
int main() {
    std::cout << "Running Quotient Routing tests..." << std::endl;

    test_quotient_routing_basic();
    test_quotient_routing_with_metrics();
    test_quotient_node_leave();

    std::cout << "All Quotient Routing tests passed." << std::endl;
    return 0;
}